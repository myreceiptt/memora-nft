...
/**
 * Get the largest token id that _is minted_ and it is works for both ERC721 and ERC1155.
 * @param options
 */
export async function getSupplyInfo(
  ...
  if (!_is1155 && !_is721) {
    throw new Error("Not an NFT collection, OiOi!");
  }
  if (_is1155) {
    ...
    const [startTokenId_, maxSupply] = await Promise.allSettled([
      ...
      // otherwise use nextTokenIdToMint
        ...
        throw new Error(
          "Contract requires either `nextTokenIdToMint` or `totalSupply` function available to determine the next token ID to mint, OiOi!"
        );
      }
